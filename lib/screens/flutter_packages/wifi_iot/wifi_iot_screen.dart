...
class WifiIotScreen extends StatefulWidget {
  final String title;
  final Object model;
  ...
  @override
  _WifiIotScreenState createState() => _WifiIotScreenState();
}

class _WifiIotScreenState extends State<WifiIotScreen> {
  ...
  void showClientList() async {
    ...
    getClientList(false, 300).then((val) => val.forEach((oClient) {
          log('************************');
          log('Client :');
          log("ipAddr = '${oClient.ipAddr}'");
          log("hwAddr = '${oClient.hwAddr}'");
          log("device = '${oClient.device}'");
          log("isReachable = '${oClient.isReachable}'");
          log('************************');
        }));
  }
